...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      log.error("Google Places autocomplete API error", new Error(`Status ${response.status}`), {
    ...
    log.error("Places autocomplete proxy error", error instanceof Error ? error : new Error(String(error)));
